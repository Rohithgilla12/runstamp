...
- **Strava quota** — Strava caps new API apps at 1 connected athlete.
  Quota-increase request in with their developer team. The whole Strava
  integration is built and tested but **hidden behind a build flag
  (`STRAVA_ENABLED`)** for the Apple-Health-only App Store launch — no Strava
  connect surface ships until the quota lands. Apple Health is the primary
  (and currently only) ingestion source. Re-enabling Strava is a one-line
  flag flip once approved.
