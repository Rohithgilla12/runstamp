...
type Sub = 'main' | 'shoes' | 'connections' | 'privacy' | 'health-runs';
...
export function SettingsScreen(_props: TabProps<'Profile'>) {
  ...
  if (sub === 'connections') {
    return (
      <ConnectionsScreen
        back={() => setSub('main')}
        goToHealthRuns={() => setSub('health-runs')}
      />
    );
  }
  if (sub === 'health-runs') return <HealthRunsScreen back={() => setSub('connections')} />;
...
function ConnectionsScreen({ back, goToHealthRuns }: { back: () => void; goToHealthRuns: () => void }) {
  const c = useColors();
...
// HealthRunsScreen — the "Browse HealthKit runs" power-user screen reached
// from Settings → Connections → Apple Health → BROWSE RUNS. Lists every
// running workout HealthKit has on this device, flagged as imported or not,
// with a one-tap retry for the not-imported ones. Useful when the auto-sync
// misses a workout (Apple Watch sync lag, permission glitch, etc).
function HealthRunsScreen({ back }: { back: () => void }) {
  const c = useColors();
  const { getIdToken } = useAuth();
  ...
  const importedUuids = React.useMemo(() => {
    ...
    async (uuid: string) => {
      ...
      try {
        ...
        if (res.uploaded === 0 && res.duplicates === 0) {
          // Server accepted the batch but neither uploaded nor flagged as
          // dupe — almost always a skipped row (bad distance or parse).
  ...
  return (
    ...
    >
      <SubHeader back={back} title="HEALTHKIT RUNS" />
      <View style={{ paddingHorizontal: 20, paddingTop: 14 }}>
