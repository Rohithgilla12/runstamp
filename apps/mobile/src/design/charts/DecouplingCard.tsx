  ...
  return (
    <Card style={{ backgroundColor: c.paper2 }}>
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <View style={{ paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, backgroundColor: tone + '22', marginTop: 4, maxWidth: 140 }}>
      ...
      <View style={{ marginTop: 12, alignItems: 'center' }}>
        <Svg width={W} height={H}>
          ...
          {/* Dots per run */}
          {series.map((p, i) => {
            const tone = p.decouplingPct < 5 ? c.moss : p.decouplingPct < 10 ? c.ink2 : c.accent;
            return <Circle key={i} cx={x(i)} cy={y(p.decouplingPct)} r={3} fill={tone} />;
          })}
