...
const EXPLANATION =
  'Banister/Coggan training-stress balance over the last 90 days.\n\n' +
  '• CTL (fitness) — rolling 42-day average of daily training load.\n' +
  '• ATL (fatigue) — rolling 7-day average.\n' +
  '• TSB (form) — CTL minus ATL. Positive means fresh, negative means loaded.\n\n' +
  'Race-ready zone is roughly +5 to +25 TSB. Below −30 is overreaching — back off.';
...
export function FormChartCard({ series, isHrBased, needsHrProfile, onTapProfile }: Props) {
  const c = useColors();
  const { captureRef, share, busy } = useChartShare('Form');
      ...
      <View ref={captureRef} collapsable={false} style={{ backgroundColor: c.paper }}>
        <Card style={{ backgroundColor: c.paper2 }}>
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            ...
            <View style={{ paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, backgroundColor: tsbTone + '22', marginTop: 4, maxWidth: 170, marginRight: 40 }}>
          ...
          <View style={{ marginTop: 12, alignItems: 'center' }}>
            <View style={{ width: W, height: H }}>
              ...
              <ChartTooltip
                ...
                height={H}
                dotColor={c.accent}
                pointY={(p) => yLoad(p.ctl)}
                formatPrimary={(p) => formatTooltipDate(p.date)}
                formatValue={(p) => `Fit ${Math.round(p.ctl)} · Fat ${Math.round(p.atl)} · TSB ${p.tsb >= 0 ? '+' : ''}${Math.round(p.tsb)}`}
              />
            </View>
          </View>
