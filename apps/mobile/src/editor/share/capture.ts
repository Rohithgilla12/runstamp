...
import type { Background, LiveStreams } from '../layouts/types';
...
export async function captureCanvas({
  canvasRef, background, live, canvasW, canvasH, tileStyle,
}: CaptureArgs): Promise<string> {
  if (background === 'map' && live.rawLatLng != null && live.rawLatLng.length > 1) {
